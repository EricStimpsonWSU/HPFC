...
def get_dc_mode(field_hat: np.ndarray) -> complex:
    ...
    Works for 2D arrays and for batched arrays where the spatial dimensions
    are the last two axes.
    """
...
def batched_fftn(payload_mgr, arr, axes: Tuple[int, ...] = (-2, -1)):
    """Perform an N-D FFT on batched fields using the given payload manager.

    The payload manager is expected to expose `fftn`.
    """
    # ensure array is in the backend namespace (CuPy/NumPy) expected by fft
...
def batched_ifftn_real(payload_mgr, arr_hat, axes: Tuple[int, ...] = (-2, -1)):
    """Perform inverse FFT and return the real part via payload manager."""
