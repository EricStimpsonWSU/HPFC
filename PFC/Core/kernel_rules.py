...
def gaussian_kernel_fft(k2: np.ndarray, *, width: float | None = None, dtype: np.dtype = np.float64) -> np.ndarray:
	if width is None:
...
class KernelRules:
	...
	def __post_init__(self) -> None:
		...
		if sim_build is None:
			...
			try:
				...
				raise AttributeError("Model is incompatible: missing required parameters for kernel construction")

			sim_build = _build_from_generic_model

		dt = self.model.dt
		gamma = self.model.Gamma
		self.lin_dpsi, self.lin_mu_kernel, self.lin_f_kernel, self.lin_v_kernel = sim_build(self.model, self.geometry)
	...
	@staticmethod
	def buildLinearETD(dt: float, lin_kernel: np.ndarray) -> np.ndarray:
		return np.exp(lin_kernel * dt)

	@staticmethod
	def buildNonlinearETD(dt: float, lin_kernel: np.ndarray) -> np.ndarray:
		nonlin_kernel = np.ones_like(lin_kernel) * dt
		nonlin_kernel[lin_kernel != 0] = (np.exp(lin_kernel[lin_kernel != 0] * dt) - 1) / lin_kernel[lin_kernel != 0]
		return nonlin_kernel

	def buildKernelsETD(self, dt: float, Gamma: float) -> None:
		...
		self.nonlin_dpsi_kernel[self.lin_dpsi != 0] = (self.lin_dpsi_exp_kernel[self.lin_dpsi != 0] - 1) / self.lin_dpsi[self.lin_dpsi != 0]
